...
#define F_CPU 16000000 // (126000 * 128) // or from freqOutDWire() -> counter;  or 18432000  
...
void SerialSync() {  // after break, reset, single step, ...
  ...
  while (1) {
		commErrs(true);
    u8 byte = 0;
    if (rxRdy()) switch (byte = SerialReadByte()) {
    ...
    default:
      ...
      u8 out = 0x55;
      WriteFile(hCom, &out, 1, NULL, NULL);
      ReadFile(hCom, &out, 1, NULL, NULL); // echo
      Sleep(1);
    } else printf("No 0x55\n");
  }
}
...
void setFastBaud() { 
  ...
  DwSend(0xA0); // set baud, expect 0x55
	txFlush();    // get echo at previous baud
  setBaudRate(F_CPU / 8); // CP2102 table 2 MHz
#else
  DwSend(0xA1); // set baud, expect 0x55
  txFlush();
	setBaudRate(F_CPU / 4); // CP2102 table 4 MHz
#endif
  SerialSync();  // may need finer control of timing to catch 0x55   TODO
}
